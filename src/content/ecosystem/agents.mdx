...
Most AI interactions are one-shot: you ask, it answers. But complex tasks require multiple steps. Research a topic, book a trip, debug a codebase—these can't be done in a single response.
...
<FlowDiagram
  ...
  steps={[
    { id: '1', label: 'Task', sublabel: 'User goal', icon: '🎯' },
    { id: '2', label: 'Reason', sublabel: 'Plan next step', icon: '🤔' },
    { id: '3', label: 'Act', sublabel: 'Call tool', icon: '🔧' },
    { id: '4', label: 'Observe', sublabel: 'Process result', icon: '👁️' },
    { id: '5', label: 'Complete?', sublabel: 'Check if done', icon: '✅' },
  ]}
...
→ Which uses: SQL tools, migration tools

Planner: "Need to update the API"
→ Delegates to: Code Agent
→ Which uses: File editing, testing tools
```
