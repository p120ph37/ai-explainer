...
<FlowDiagram
  ...
  steps={[
    { id: '1', label: 'User Query', sublabel: 'What\'s the weather?', icon: '💬' },
    { id: '2', label: 'Model Decides', sublabel: 'Need tool: get_weather', icon: '🤔' },
    { id: '3', label: 'Tool Call', sublabel: 'Execute function', icon: '🔧' },
    { id: '4', label: 'Result', sublabel: 'Data returned', icon: '📥' },
    { id: '5', label: 'Response', sublabel: 'With real info', icon: '✅' },
  ]}
  direction="horizontal"
  ariaLabel="Flow diagram showing the tool use cycle from query to response"
/>

<Recognition title="Let me search for that">
...
- Requests involving files → file system tools
- Code that needs testing → code execution
...
<Question title="What are agents?">

Tool use enables **agents**: systems that iteratively plan, act, observe, and repeat until a goal is achieved.

Instead of one tool call, an agent might make dozens (searching, analyzing, deciding, acting) in a loop driven by the model's reasoning. A research task might involve many searches; a booking might require search, comparison, and confirmation steps.

Tool use is the capability. Agency is the pattern that uses it across many steps.
