...
<FlowDiagram
  ...
  steps={[
    { id: '1', label: 'Tokenize Input', sublabel: 'Text → Token IDs', icon: '📝' },
    { id: '2', label: 'Forward Pass', sublabel: 'Through all layers', icon: '🔄' },
    { id: '3', label: 'Get Probabilities', sublabel: 'For next token', icon: '📊' },
    { id: '4', label: 'Sample Token', sublabel: 'Based on temperature', icon: '🎲' },
    { id: '5', label: 'Append & Repeat', sublabel: 'Until done', icon: '➡️' },
  ]}
...
<Recognition title="Streaming is the loop made visible">
...
Processing input through the network. Data enters at the input layer, flows through each layer in sequence (embedding → attention → feed-forward, repeated many times), and exits at the output layer. Each layer transforms the representation.

"Forward" because data moves input → output. Training has a "backward pass" where errors flow output → input to compute gradients.
...
The speedup is dramatic: what would be O(n²) becomes O(n) for the sequential generation case. The tradeoff: the cache consumes memory. Long contexts with large caches can exhaust GPU memory.
